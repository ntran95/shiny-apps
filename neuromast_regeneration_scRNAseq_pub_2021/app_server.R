server <- function(input, output) {
  ...
  ClusterPlotF <- function() {
    ...
    if ("cell.type.ident" %in% colnames(seurat_obj@meta.data)) {
      umap_clusters <- DimPlot(seurat_obj, reduction = "umap", pt.size = 0.10,
                               ...
                               cols = cell_type_colors$HEX.colors)
    } else {
      umap_clusters <- DimPlot(seurat_obj, reduction = "umap", pt.size = 0.10,
  ...
  DatasetPlotF <- function() {
    ...
    umap_dataset <- DimPlot(seurat_obj, reduction = "umap", pt.size = 0.10,
                            label = TRUE, label.size = 0, group.by = "data.set",
                            cols = trt_colors)
...
# === redirect to Pseudotime Shiny app
# observeEvent(input$panel,{
#   if(input$panel == "pseudotime"){
#     js$browseURL("https://piotrowskilab.shinyapps.io/pseudotime_regen_traj_Sungmin/",
...
# })
  ...
  # observeEvent(input$panel,{
  #   if(input$panel == "pseudotime"){
  #     browseURL("https://piotrowskilab.shinyapps.io/pseudotime_regen_traj_Sungmin/",
  #               browser = getOption("browser"),encodeIfNeeded = T)
  #   }
  # })
  
  # output$Link <- renderUI({
  #   
  #   a("test",href="https://piotrowskilab.shinyapps.io/pseudotime_regen_traj_Sungmin/", target="_blank")
  #   
  # })
} # Server close
